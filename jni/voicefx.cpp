 ...
 * Algoritma: simple in-place resample (tanpa ring buffer)
 */
...
// Buffer statis untuk copy input
#define MAX_BUF 8192
...
// ============================================================
// DSP CALLBACK - simple resample in-place
// ============================================================
...
static void  _vc_set_pitch(float f) {
    ...
    g_pitch = f;
    char tmp[64];
    snprintf(tmp, sizeof(tmp), "[VFX] pitch=%.2f", f);
    logf(tmp);
}
static void  _vc_enable(void)     { g_enabled = 1; logf("[VFX] enabled"); }
static void  _vc_disable(void)    { g_enabled = 0; logf("[VFX] disabled"); }
...
void OnModLoad() {
    ...
    if (!hDobby) { logf("[VFX] ERROR: libdobby gagal"); return; }
    ...
    if (!hBASS) { logf("[VFX] ERROR: libBASS gagal"); return; }
    ...
    if (r != 0) { logf("[VFX] ERROR: DobbyHook"); return; }

    g_pitch   = 1.0f;
    g_enabled = 0;

    // Tulis alamat vc_api untuk Lua
